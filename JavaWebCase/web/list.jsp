...
<head>
    ...
    <script>
        ...
        window.onload =function () {
            ...
            document.getElementById("fistid").onclick = function () {
                var ids = document.getElementsByName("uid");
                for (var i = 0; i <ids.length ; i++) {
...
<div class="container">
    ...
    <div style="float: left;">
        ...
        <form class="form-inline" action="${pageContext.request.contextPath}/FindUserByPageServlet" method="post">
            ...
            <div class="form-group">
                ...
                <input type="email" class="form-control" name="email" value="${map.email[0]}" id="exampleInputEmail2"  >
            </div>
    ...
    <table border="1" class="table table-bordered table-hover">
        <tr class="success">
            <th><input type="checkbox" id="fistid"></th>
            <th>编号</th>
            <th>姓名</th>
            <th>性别</th>
            <th>年龄</th>
        ...
        <c:forEach items="${bean.list}" var="user" varStatus="s">
            <tr>
                ...
                <td>${user.qq}</td>
                <td>${user.email}</td>
                <td><a class="btn btn-default btn-sm" href="${pageContext.request.contextPath}/FindById?id=${user.id}">修改</a>&nbsp;
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <c:if test="${bean.currentPage==1}">
                    <li style="pointer-events: none"><%--此标签不支持任何事件--%>
                </c:if>
                <c:if test="${bean.currentPage!=1}">
                    <li>
                </c:if>
                    <a href="${pageContext.request.contextPath}/FindUserByPageServlet?currentPage=${bean.currentPage-1}&rows=5&name=${map.name[0]}&address=${map.address[0]}&email=${map.email[0]}" aria-label="Previous">
                ...
                <c:if test="${bean.currentPage!=bean.totalPage}">
                    ...
                    <a href="${pageContext.request.contextPath}/FindUserByPageServlet?currentPage=${bean.currentPage+1}&rows=5&name=${map.name[0]}&address=${condition.address[0]}&email=${condition.email[0]}" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
                <span style="font-size: 25px;margin-left: 5px;">
                    共${bean.totalCount}条记录，共${bean.totalPage}页
                </span>
